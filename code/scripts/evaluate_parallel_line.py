...
def evaluate(**deps):
    ...
    env_list = [Find_Dot(max_number_steps=50) for _ in range(num_eval)]
    dones = [0 for _ in range(num_eval)]
    episode_rewards = [0 for _ in range(num_eval)]

    assert trainer.ema_model.condition_guidance_w == Config.condition_guidance_w
    returns = to_device(100 * torch.ones(num_eval, 1), device)
    ...
    while sum(dones) <  num_eval:
        ...
        for i in range(num_eval):
            ...
            this_obs, this_reward, this_done, this_infos = env_list[i].step(action[i])
            success += this_infos['goal_reached']
            print(f"observations: {this_obs}")
            if this_done:
                if dones[i] == 1:
                    pass
                else:
        ...
        print(f"t: {t}, episode_rewards: {episode_rewards}, dones: {dones}, success: {success/num_eval}")
