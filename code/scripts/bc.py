...
path ="/home/fernandi/projects/decision-diffuser/code/trajectories/safe_grid_2000_2_noise.pickle"
#read pickle
...
class TrainConfig:
    ...
    seed: int = 69  # Sets Gym, PyTorch and Numpy seeds
    eval_freq: int = int(1e4)  # How often (time steps) we evaluate
    n_episodes: int = 10  # How many episodes run during evaluation
    max_timesteps: int = int(1e6)  # Max time steps to run environment
    checkpoints_path: Optional[str] = "/home/fernandi/projects/safety-gym/safety_gym/log/BC-SafeGrid-v0}"  # Save path
    load_model: str = ""  # Model load file name, "" doesn't load
    batch_size: int = 256  # Batch size for all networks
    discount: float = 0.99  # Discount factor
    # BC
    buffer_size: int = 4_054_000  # Replay buffer size
...
def eval_actor(
    ...
    for _ in range(n_episodes):
        ...
        # print(f"Success count: {success_counter}")
        # env.render()
    ...
    actor.train()
    return np.asarray(episode_rewards), np.asarray(episode_costs), success_counter/n_episodes
...
def train(config: TrainConfig):
    ...
    # )
    #env_ = wrap_env(env, state_mean=state_mean, state_std=state_std)
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            eval_scores, eval_costs, sucess_rate = eval_actor(
                env,
                actor,
                device=config.device,
            ...
            print(
                ...
                f" Score: {normalized_eval_score:.3f}, Cost: {normalized_eval_cost:.3f}, Success rate: {sucess_rate:.3f}"
            )
            print("---------------------------------------")
            ...
            wandb.log(
                {"return": normalized_eval_score, "cost": normalized_eval_cost, "success_rate": sucess_rate},
...
def eval(config: TrainConfig):
    ...
    eval_scores, eval_costs, success_rate = eval_actor(
        env,
        actor,
        device=config.device,
        n_episodes=3,
    ...
    print(
        f"Evaluation over {config.n_episodes} episodes: "
        f" Score: {normalized_eval_score:.3f}, Cost: {normalized_eval_cost:.3f}, Success rate: {success_rate:.3f}"
    )
    print("---------------------------------------")
