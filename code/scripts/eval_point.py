...
def evaluate(**deps):
    ...
    env = Find_Dot(max_number_steps=50,start_x=30,start_y=30)
    env_list = [env]
    ...
    while not done:
        with torch.no_grad():
            ...
            if args.conditioning =="alternating":
                if t % 2 == 0:
                    skills = (torch.tensor([[1.0,0.0]],dtype=torch.float32,device=device))
                    samples = trainer.ema_model.conditional_sample(conditions, skills=skills)
                    print("right")            
                else:
                    skills = (torch.tensor([[0.0,1.0]],dtype=torch.float32,device=device))
                    samples = trainer.ema_model.conditional_sample(conditions, skills=skills)
                    print("up")
            ...
            df['x'] = samples_unnorm[0][:,0]
            df['y'] = samples_unnorm[0][:,1]
            df.to_csv(f'/home/fernandi/projects/decision-diffuser/code/skills/analysis/experiments/30_xycond_full_{args.conditioning}_{args.seed}_w{Config.condition_guidance_w[0]}.csv', index=False)
            ...
            diff_x = samples[0][:,0][1] -samples_unnorm[0][:,0][0]
